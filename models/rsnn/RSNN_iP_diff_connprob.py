...
class RSNN_iP(nn.Module):
    def __init__(self):
        super(RSNN_iP, self).__init__()
        num_inputs = 3
        num_hidden = 200
        num_output = 1
        pe_e = 0.16

        p_nn = {'e_e': 0.16, 'e_PV': 0.395,
                'PV_e': 0.411, 'PV_PV': 0.451
                } 
        ...
        num_iPV = num_hidden - num_excitatory
        ...
        beta = torch.cat((beta_e, beta_iPV)) # create array of betas corresponding to each neuron!
        ...
        hidden_mx = hid_mx_iPV(num_excitatory, num_iPV, p_nn) 
