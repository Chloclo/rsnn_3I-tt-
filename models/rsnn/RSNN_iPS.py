...
class RSNN_iPS(nn.Module):
    def __init__(self):
        super(RSNN_iPS, self).__init__()
        num_inputs = 3
        num_hidden = 200
        num_output = 1
        pe_e = 0.16

        p_nn = {'e_e': 0.16, 'e_PV': 0.395, 'e_SST': 0.182,
                'PV_e': 0.411, 'PV_PV': 0.451, 'PV_SST': 0.03, 
                'SST_e': 0.424, 'SST_PV': 0.857, 'SST_SST': 0.082, 
                } 
        ...
        # Three inhibitory neuron classes: 60% PV, 40% SST
        num_iPV = round(0.6 * num_inhibitory)
        num_iSST = num_inhibitory-num_iPV
        ...
        beta_iPV = torch.asarray([iPV_beta] * num_iPV)
        beta_iSST = torch.asarray([iSST_beta] * num_iSST)
        
        beta = torch.cat((beta_e, beta_iPV, beta_iSST)) # create array of betas corresponding to each neuron!
        ...
        hidden_mx = hid_mx_iPS(num_excitatory, num_iPV,num_iSST, p_nn) 
    ...
    def positive_negative_weights(self):
        ...
        self.rlif1.recurrent.weight.data = weight_matrix.view(self.rlif1.recurrent.weight.data.shape)
